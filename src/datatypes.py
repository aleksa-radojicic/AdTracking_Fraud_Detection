...
class Filepaths:
    ...
    train_subset: Path = Path("~/Projects/AdTracking_Fraud_Detection/data/train_subset.parquet")
    test_subset: Path = Path("~/Projects/AdTracking_Fraud_Detection/data/test_subset.parquet")
...
class ExtendedSchema(pa.DataFrameModel):
    click_counts_group: pl.UInt8  # Group 0: clicks between [ip_counts.min(), ip_counts.quantile(0.75)+1), Group 1: clicks between [ip_counts.quantile(0.75)+1, ip_counts_above_3rd_quantile.quantile(0.75)+1), Group 2: clicks >= ip_counts_above_3rd_quantile.quantile(0.75)+1
